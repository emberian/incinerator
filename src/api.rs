...
impl<Hdr: 'static> HeapRegistration<Hdr> {
    ...
    /// Very bad!
    pub unsafe fn alloc<'a, T: crate::Trace + Sync + 'static>(
        &self,
        val: T,
        ...
        latest.map(|x| {
            ...
            let vtable = std::ptr::metadata::<dyn crate::Trace>(x.cast::<T>().as_ptr());
                ...
                .find(|(_, &vt)| Some(vtable) == vt)
            {
                ...
                None => {
                    ...
                    (*seg).trace_tables[(*seg).trace_count.get() as usize] = Some(vtable);
